import {Rapports} from '$lib/types/Rapport';
...
export const load = async ({fetch}) => {
  ...
  `;
    ...
    const base = "http://localhost:4000/api/graphql"
    const mVariables = {
        "page": 1,
        "limit": 12,
        "sort": "-createdAt",
...
// }
    ...
    const resNews = await fetch(`${base}`, {
        method: 'POST',
        headers: {'content-type': 'application/json'},
        body: JSON.stringify({newsQuery, nVariables})
    });
    ...
    console.log(JSON.stringify(jsonNews.data, null, 3));
    ...
    return {rapports: json.data as Rapports, news: jsonNews.data as Annonces}
}
